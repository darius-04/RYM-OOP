    ...
    try
    {
        ...
        do
        {
            ...
            case 8:
            {
                    ...
                    if (auto artist = std::dynamic_pointer_cast<Artist>(obiect))
                    {
                        for (const auto& album : artist->getAlbume())
                        {
                            for (const auto& melodie : album->getMelodii())
                            {
                                ...
                                std::cout << indexMelodie++ << ". " << melodie->getNume() << " (Album: " << album->getTitlu() << ", Artist: " << artist->getNume() << ")\n";
                            }
                        }
                    }
                }
                ...
                std::cout << "Introdu comentariul: ";
                std::getline(std::cin >> std::ws, comentariu);
                nota = citesteNumar<double>("Introdu nota (intre 1 si 10): ");
