...
void Melodie::adaugaRecenzie(std::shared_ptr<Recenzie<double>> recenzie) {
    recenzii.push_back(recenzie);
}

void Melodie::afiseazaDetalii() const {
    std::cout << "Melodie: " << nume << " (Nota: " << std::fixed << std::setprecision(2) << getNotaMedie() << ")";
...
const std::vector<std::shared_ptr<Recenzie<double>>>& Melodie::getRecenzii() const {
    return recenzii;
}
...
std::string Melodie::getTipObiect() const {
    return "Melodie";
}
